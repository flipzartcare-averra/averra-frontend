...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-road pb-16">
      <Head>
        <title>Admin dashboard — Averra</title>
      </Head>
            ...
            {bookings.map((b) => (
              ...
              >
                ...
                {b.estimatedFare != null && (
                  <p className="font-mono text-sm text-meter font-bold mb-1">
                    ₹{Math.round(b.estimatedFare).toLocaleString("en-IN")}
                  </p>
                )}

                {b.paymentStatus === "advance_paid" ? (
                  <p className="font-mono text-[10px] text-meter mb-3">
                    ✓ Advance paid via Razorpay
                    {b.advanceAmount != null && ` — ₹${Math.round(b.advanceAmount).toLocaleString("en-IN")}`}
                  </p>
                ) : b.paymentStatus === "failed" ? (
                  <p className="font-mono text-[10px] text-alert mb-3">✗ Advance payment failed</p>
                ) : (
                  <p className="font-mono text-[10px] text-steel mb-3">No advance paid</p>
                )}
